var _helper = _helper || (function () {
    ...
    return {
        transformName: addVendorPrefix('transform')
    }
})();
...
(function (e) {
    ...
    function h(e) {
        if ((new Date).getTime() < r + n) return;
        r = (new Date).getTime();
        var t = s.offset() != null ? s.offset().left : 0,
            u = s.offset() != null ? s.offset().top : 0,
            a = e.pageX - t,
            h = e.pageY - u;
        if (a < 0 || a > s.width() || h < 0 || h > s.height()) return;
        if (l()) {
        ...
        for (m = i.length; m--;) {
            v = i[m];
            newX = v.inversionFactor * v.xRange * p;
            newY = v.inversionFactor * v.yRange * d;
            if (v.background) {
                // todo
                v.obj.css("background-position", (v.startX + newX) + "px " + (v.startY + newY) + "px");
            } else {
                //todo instead v.obj.css("left", newX).css("top", newY)
                v.obj[0].style[_helper.transformName] = 'translate3d(' + newX + 'px, ' + newY + 'px, 0)';
            }
        }
    }
    ...
    e.plax = {
        ...
        disable: function (t) {
            ...
            if (t && typeof t.clearLayers === "boolean" && t.clearLayers) i = []
        }
    };
    ...
    if ($('#auth').length) {
        ...
        $(document).on('keydown', function (event) {
            if (event.target === document.body && event.keyCode === 192 && !event.metaKey) {
                ...
                setTimeout(function () {
                    window.location = '/site/toggle_staff'
                }, 250)
                return false;
            }
        })
